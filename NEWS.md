...
tables for all possible pairs of lists.
...
- __sorenThreshold__: Implement an algorithm allowing the computation of the “equivalence thresholds” dissimilarities matrix for all the k(k–1)/2 tests. According to what is indicated in the arguments, the results of this function are stored in a matrix for a specific ontology and level or a list with a matrix for more than one ontology and/or level.
...
- __allHclustThreshold__: Performs the same calculations as hclustThreshold but for
all three ontologies PB, MF, and CC and levels from 2 to 10

...
#' representing a 2x2 contingency table,
#' or a "character" (a set of gene identifiers) or "list" or "tableList" object.
#' See the details section for more information.
#' @param y an object of class "character" representing a vector of gene
#' identifiers (e.g., ENTREZ).
#' @param dis Sorensen-Dice dissimilarity value. Only required to speed
#' computations if this value is known in advance.
#' @param se standard error estimate of the sample dissimilarity. Only required
#' to speed computations if this value is known in advance.
#' @param conf.level confidence level of the one-sided confidence interval, a
#' numeric value between 0 and 1.
#' @param z.conf.level standard normal (or bootstrap, see arguments below)
#' distribution quantile at the \code{1 - conf.level} value. Only required to
#' speed computations if this value is known in advance.
#' Then, the argument \code{conf.level} is ignored.
#' @param boot boolean. If TRUE, \code{z.conf.level} is computed by means of a
#' bootstrap approach instead of the asymptotic normal approach. Defaults to
#' FALSE.
...
#' to adequately represent a 2x2 contingency table. This checking is performed
#' by means of function \code{nice2x2Table}.
#' @param ... additional arguments for function \code{buildEnrichTable}.
#'
#' @return In the "table", "matrix", "numeric" and "character" interfaces,
#' the value of the Upper limit of the confidence interval for the Sorensen-Dice
#' dissimilarity. When \code{boot == TRUE}, this result also haves a an extra
...
#'
#' @details
#' This function computes the upper limit of a one-sided confidence interval for
#' the Sorensen-Dice dissimilarity, given a 2x2 arrangement of frequencies
#' (either implemented as a "table", a "matrix"
#' or a "numeric" object):
#'
#' \tabular{rr}{
...
#' if necessary.
#'
#' Arguments \code{dis}, \code{se} and \code{z.conf.level} are not required.
...
#'
#' By default, \code{z.conf.level} corresponds to the 1 - conf.level quantile of
...
#' initially planned bootstrap samples \code{nboot}.

#' Computing in advance the value of argument \code{z.conf.level} may be a way
#' to cope with these departures from normality, by means of a more adequate
#' quantile function. Alternatively, if \code{boot == TRUE}, a bootstrap
...
#' (e.g., ENTREZ).
#' Then the confidence interval for the dissimilarity between lists \code{x} and
#' \code{y} is computed, after internally summarizing them as a 2x2 contingency
...
#' \code{dUppSorensen}.
#'
#' In the "list" interface, the argument must be a list of "character" vectors,
#' each one representing a gene list (character identifiers). Then, all pairwise
#' upper limits of the dissimilarity between these gene lists are computed.
#'
...
#' function \code{\link{buildEnrichTable}}) is a list of lists of contingency
...
#' dissimilarity, \code{\link{equivTestSorensen}} for an equivalence test.
#'
#' @examples
...
duppSorensen.table <- function(x,
                               ...
                               check.table = TRUE, ...) {
  stopifnot(
    "Argument'conf.level' must be numeric between 0 and 1" =

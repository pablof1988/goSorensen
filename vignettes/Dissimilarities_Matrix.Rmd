...
- name: Alex Mantilla
  email: asmantilla.25@est.ucab.edu.ve
  affiliation: Universidad Catolica Andres Bello (UCAB), Facultad de Ingeniería, Doctorado en Ingeniería.   
header-includes:
  -\usepackage{amsmath}
...
- Step 2. According to a criterion of type I error control in multiple testing, let $\mathfrak{d}_h$ be the smallest irrelevance limit for which all $h$ null hypothesis are rejected. The default adjusting criterion in `goSorensen` is the Holm's method ("holm" option in the R function `p.adjust`). So, $\mathfrak{d}_h$ is computed as: $$\mathfrak{d}_h = \text{min} \left(\mathfrak{d}: P_{(I)}(\mathfrak{d}) \leq \frac{\alpha}{h+1-I}; \hspace{0.5cm} I=1, 2, \ldots,h \right).$$
...
## For a Specific Ontology, with or without GO level restriction.
...
The dissimilarity matrix can also be computed without restricting the analysis to a specific GO level by setting `GOLevel = NULL`. For example, for the BP ontology:

```{r dissMatrx-BP, eval=FALSE}
...
In this case, the dissimilarity matrix is computed considering all GO terms in the selected ontology, without GO level restriction.
...
```{r cont_all_BP4, warning=FALSE, message=FALSE, comment=NA, eval=FALSE}
...
Similarly, if the enrichment contingency tables were previously computed without GO level restriction, the dissimilarity matrix can be obtained as follows:

```{r dissMatrx_BP,eval=FALSE}
...
```{r boot_dissMatrx-BP4, eval=FALSE}
...
## For More than One Ontology, with or without GO level restriction.
...
```{r allDissMatrx, warning=FALSE, message=FALSE, comment=NA, eval=FALSE}
...
The dissimilarity matrices can also be computed without restricting the analysis to specific GO levels by setting `GOLevels = NULL`:

```{r allDissMatrxNoLevel,eval=FALSE}
...
In this case, one dissimilarity matrix is obtained for each selected ontology, without subdivision by GO levels.
...
```{r allContTabs, warning=FALSE, message=FALSE, comment=NA, eval=FALSE}
# Previously compute the enrichment contingency tables:
...
The same computation can be performed from precomputed contingency tables obtained without GO level restriction:

```{r allContTabsNoLevel, eval=FALSE}
...
```{r clust.threshold, warning=FALSE, message=FALSE}
...
```{r plot, warning=FALSE, message=FALSE, fig.align='left', fig.height=3, fig.width=5}
library(ggplot2)
library(ggrepel)
graph <- ggplot() +
...
```{r graph, warning=FALSE, message=FALSE, fig.align='left', fig.height=3, fig.width=5}
graph +
  geom_vline(
...
```{r mean_sd, warning=FALSE, message=FALSE}
...
$$st_j = \dfrac{| _L\overline{X}_{GO_j} - _R \overline{X}_{GO_j}|}{\sqrt{\dfrac{_LS^2_{GO_j}}{l} + \dfrac{_RS^2_{GO_j}}{r} + \epsilon  }}$$ with $l$ the number of lists to the extreme left and $r$ the number of lists to the extreme right. Using $\epsilon= 0.00000001$ the statistic is computed as follows:

```{r nl, warning=FALSE, message=FALSE}
nl <- ncol(tableleft)
nr <- ncol(tableright)

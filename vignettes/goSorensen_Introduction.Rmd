...
```{r bioc, message = FALSE, warning = FALSE, eval = FALSE}
if (!requireNamespace("org.Hs.eg.db", quietly = TRUE)) {
  BiocManager::install("org.Hs.eg.db")
}
```

```{r org, message = FALSE, warning = FALSE, eval = TRUE}
...
```{r human, message = FALSE, warning = FALSE, echo = TRUE}
...
## For One List (With or Without GO Level Restriction).

The first step[^1] is to determine whether the GO terms of a specific ontology are enriched or not enriched across the different lists to be compared. This can be done either at a specific GO level or without GO level restriction. The `enrichedIn` function assigns `TRUE` when a GO term is enriched in a gene list and `FALSE` when it is not.
...
## For Two or More Lists (With or Without GO Level Restriction).
...
```{r cont-atlas-sanger-BP4, eval=FALSE}
...
```{r cont-b-atlas-sanger-BP4, echo=FALSE}
...
```{r cont-atlas-sanger-BP,eval=FALSE}
...
```{r cont-b-atlas-sanger-BP,echo=FALSE}
...
```{r cont-b-all-BP4,echo=FALSE, eval=FALSE}
data("cont_all_BP4")
cont_all_BP4
```
...
```{r cont-b-all-BP,echo=FALSE,eval=FALSE}
data("cont_all_BP")
cont_all_BP
```
...
## For More than One Ontology and GO Configurations (Levels or No Level Restriction).
...
```{r eqTest-atlas-sanger-BP4, eval=FALSE}
...
```{r eqTest-b-atlas-sanger-BP4, echo=FALSE}
...
```{r eqTest-atlas-sanger-BP,eval=FALSE}
...
```{r eqTest-b-atlas-sanger-BP,echo=FALSE}
...
```{r eqTest-db-all-BP4, echo=FALSE}
data(eqTest_all_BP4)
```
...
## For Multiple Ontologies and GO Configurations.
...
```{r alleqTestsNoLevel,eval=FALSE}

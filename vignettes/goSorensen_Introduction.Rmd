...
package: goSorensen
abstract: >
  This vignette provides an introduction to goSorensen [@goSorensen] package, which was built to determine equivalence between features lists. The method is based on the Sorensen–Dice index and the joint frequencies of GO term enrichment. 
...
# Data.
The dataset used in this vignette, `allOncoGeneLists`, is based on the gene lists compiled at <http://www.bushmanlab.org/links/genelists>, a comprehensive set of gene lists related to cancer. The package
`goSorensen` loads this dataset by means of `data(allOncoGeneLists)`:
```{r}

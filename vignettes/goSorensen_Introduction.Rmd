...
package: goSorensen
abstract: >
  This vignette provides an introduction to goSorensen [@goSorensen] package, which is built to determine equivalence between feature lists based on the Sorensen–Dice index and the joint frequencies of GO term enrichment. 
  
  Starting from an introduction of the asociated technique and a description of the data used, We explain: i) the way to perform the equivalence test from contingency tables or directly from feature lists (either using an asymptotic theory or a bootstrap approximation), ii) results extracted from a group of functions that allows us to collect specific fields of the tests such as the p-value, upper limit of the confidence interval, studentized statistics, standard error, etc..., and iii) Another statistics related to the Sorensen-Dice dissimilarity
...
# Data.
`allOncoGeneLists` is based on the gene lists compiled at <http://www.bushmanlab.org/links/genelists>, a comprehensive set of gene lists related to cancer

`goSorensen` load this data compiling `data(allOncoGeneLists)`:
```{r}
...
# Another statistics related to the Sorensen-Dice dissimilarity
